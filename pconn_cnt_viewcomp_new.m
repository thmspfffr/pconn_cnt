...
load /home/tpfeffer/pconn_cnt/proc/src/pconn_cnt_sa_s4_m1_b1_v1.mat
total_err = 0;
  ...
  while back == 1
    ...
    for is = 1 : 20
      ...
      for ibl = 1 : 2;
        for ifr = 1 : 2;
        cnt = cnt + 1;
        a(cnt) = exist(sprintf([outdir 'pconn_cnt_rejected_comps_s%d_m%d_b%d_f%d_v%d.mat'],is,im,ibl,ifr,v));
      end; 
      end
      end
      if ~any(a<1)
      ...
      subj{is} = eval(sprintf('uicontrol(''Units'',''normalized'',''Position'',[-0.05+is/20 0.90 0.05 0.05],''Style'',''pushbutton'',''Backgroundcolor'',subj_col{ex(im,iblock,is)+1},''String'',''S%d'',''Callback'',''isubj = %d'')',is,is));
    end
    ...
    for is = 21 : NSUBJ
      ...
      subj{is} = eval(sprintf('uicontrol(''Units'',''normalized'',''Position'',[-0.05+(is-20)/20 0.85 0.05 0.05],''Style'',''pushbutton'',''Backgroundcolor'',subj_col{ex(im,iblock,is)+1},''String'',''S%d'',''Callback'',''isubj = %d; clear comp_var'')',is,is));
    end
...
%     for ises = 1 : 3
%       if exist(sprintf([outdir 'pconn_cnt_ted_comps_s%d_m%d_b%d_v%d.mat'],isubj,ises,1,v)) && exist(sprintf([outdir 'pconn_cnt_rejected_comps_s%d_m%d_b%d_v%d.mat'],isubj,ises,2,v)) 
%         sescol{ises} = 'g';
%       else
%         sescol{ises} = '';
%       end
%     end
    ...
    % LOAD DATA
    load([indir sprintf('pconn_cnt_preproc_ica_s%d_m%d_b%d_v%d.mat',isubj,im,iblock,v)])
  ...
  load([indir sprintf('pconn_cnt_preproc_viewcomp_vars_s%d_m%d_b%d_f%d_v%d.mat',isubj,im,iblock,which_freq,v)])
...
%     [~,a,chanidx]=intersect(comp_low.topolabel,sa.sens_indi.label(1:268))
    
        eval(sprintf('ft_topoplotIC(cfg,comp_%s);',hilow));

%     eval(sprintf('showfield(comp_%s.topo(chanidx,i),sa.locs_2D(chanidx,:),pars);',hilow));
